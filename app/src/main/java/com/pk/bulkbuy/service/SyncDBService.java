...
public class SyncDBService extends IntentService {
    ...
    private void fetchAndProcessData(){
        ...
        final DatabaseReference ordersRef = database.getReference().child("Orders").child(sessionEmail);
        if (sessionEmail!="" && sessionEmail!=null){
            ordersRef.addValueEventListener(new ValueEventListener() {
                DB_Handler db_handler = new DB_Handler(getApplicationContext());
                @Override
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    List<Cart> storedOrders = db_handler.getOrders(sessionManager.getSessionData(Constants.SESSION_EMAIL));
                    ...
                    db_handler.insertOrderHistory(orders,sessionManager.getSessionData(Constants.SESSION_EMAIL));
                }
